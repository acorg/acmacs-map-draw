...
PYBIND11_MODULE(acmacs_map_draw_backend, m)
{
    ...
    py::class_<ChartDraw>(m, "ChartDraw")
            ...
            .def("draw", [](const ChartDraw& chart_draw, std::string filename, double size, bool timer) { chart_draw.draw(filename, size, timer ? report_time::Yes : report_time::No); }, py::arg("filename"), py::arg("size"), py::arg("timer") = false)
