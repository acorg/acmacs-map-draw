...
void ModBlobs::apply(ChartDraw& aChartDraw, const rjson::value& /*aModData*/)
{
    ...
    const auto antigen_indexes = SelectAntigens(verbose ? SelectAntigensSera::verbose::yes : SelectAntigensSera::verbose::no).select(aChartDraw, select.is_null() ? rjson::value{"all"} : select);
