...
void acmacs::mapi::v1::Settings::add_label(size_t index, size_t index_base, const rjson::v3::value& label_data)
{
    ...
    if (const auto color = rjson::v3::read_color(substitute(label_data["color"sv])); color.has_value())
        label.color(*color);
    if (const auto size = rjson::v3::read_number<Pixels>(substitute(label_data["size"sv])); size.has_value())
        label.size(*size);
    if (const auto weight = rjson::v3::read_string(substitute(label_data["weight"sv])); weight.has_value())
        label.weight(*weight);
    if (const auto slant = rjson::v3::read_string(substitute(label_data["slant"sv])); slant.has_value())
        label.slant(*slant);
    if (const auto font_family = rjson::v3::read_string(substitute(label_data["font_family"sv])); font_family.has_value())
